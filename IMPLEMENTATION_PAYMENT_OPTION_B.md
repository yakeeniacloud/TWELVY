...
- **13 Jun — ✅ FRONTEND COMMITTED + PUSHED:** branch `payment-option-b`, commit `d824200` (12 files: paymentToken, create-prospect, status, /paiement/*, inscription page, php/psp-copie/*, this doc). `npm run build` exit 0. NOT merged to main → www.twelvy.net still runs the old iframe. Vercel preview building for the branch.
  - **NEXT (needs user go):** merge `payment-option-b` → main so www.twelvy.net serves the new flow AND the /paiement/confirmation page (twelvy_validate.php redirects to www.twelvy.net hardcoded → a preview alone can't complete the confirmation). twelvy.net is staging/noindexed — safe. Then browser E2E: pick a 15-Jun stage → form → Payer → card 4000 0000 0000 1000 exp 01/27 cvc 123 → 3DS → confirmation.
  - **Open risk for E2E:** PPPS recette auth uses merchant 222 / SITE 1999888 / public test key. If the bank rejects at capture, we need the exact current Verifone recette credentials (Kader/back-office).

- **13 Jun — ✅ MERGED TO MAIN + DEPLOYED to www.twelvy.net (staging):** fast-forwarded remote main 7414ab5→d824200 (`git push origin payment-option-b:main`). Vercel deploy LIVE: `/paiement/confirmation` now 200 (was 404), inscription page 200. **Verified live:** POST www.twelvy.net/api/payment/create-prospect → `{success, stagiaire_id, redirect_url: https://psp-copie.twelvy.net/twelvy_payment.php?s&exp&sig}` → BRIDGE_API_KEY confirmed set on Vercel, signed-redirect chain works on production staging. (Local `main` still at 7414ab5 due to unrelated uncommitted working-tree files; remote main is correct — `git fetch && git reset --hard origin/main` to sync once those are handled.)
  - **STATE: fully deployed + verified up to the bank handoff. Only remaining = the browser card test (3DS is interactive) + prod cutover.**
  - Rollback the deploy: `git push origin 7414ab5:main --force` (reverts www.twelvy.net to the old iframe build).

- **13 Jun — 🐛➜✅ FIXED the "Veuillez patienter… forever" hang AND the success path (full server-side debug session).**
  - **Symptom:** form → card page renders fine → enter card → "Payer" → lands on a page with the animated `loading.gif` + "Veuillez patienter…" that spins FOREVER; nothing happens; customer never charged.
  - **Diagnosis:** drove the WHOLE chain via curl (create-prospect → twelvy_payment → /twelvy_pay_available → RemoteMPI → twelvy_validate) + a temporary `register_shutdown_function` fatal-logger on `twelvy_validate.php`, replaying real transactions and reading `error_get_last()`. RemoteMPI returns an auto-submitting form (`onload=…submit()`) POSTing `ID3D/StatusPBX/Check` to `twelvy_validate.php`; the "Veuillez patienter…" HTML is validate's OWN header (lines 35-39), flushed BEFORE its PHP runs. Browser sits on it until validate responds — and validate was dying mid-PHP (display_errors forced off on OVH → silent).
  - **ROOT CAUSE #1 — cascade of silent require_once fatals** (PSP post-payment layer require_once's files NOT deployed on psp-copie, fataling AFTER the HTML flush so the redirect `<script>` never ran): (1) `ROOT/mails_v3/mail_echec_paiement.php`; (2) `SendAdminTransactionSuccessInError.php` → file-scope `ROOT/mails_v3/functions.php`; (3) `SendPaymentSuccessEmail->__invoke` → `ROOT/mails_v3/mail_inscription.php` (success path); (4) `Logging::__construct` → `ROOT/modules/module.php` (→ Composer `vendor/` + `\Core\Log`) — on the CRITICAL path (every `new Logging()/LogPayment()/LogCommission()`).
  - **FIX #1 (hardened `twelvy_validate.php`, in repo):** email-sender classes loaded LAZILY at call sites (not top); ALL email/SMS/admin/tracking side-effects best-effort + **skipped in sandbox (`if(!DEBUG)`)** + try/catch → a missing template / dead SMTP can never hang or crash the handler. CORE path (3DS→PPPS auth+capture→`UpdateStagePaymentData`→redirect) always runs. Deployed a tiny `/www/modules/module.php` shim (no-op `\Core\Log` + `LOGS`/`DS`; no Composer/file-writes/hang). Shim source kept at `/tmp/module.php` — **add to repo as `php/psp-copie/modules-module.php`.**
  - **ROOT CAUSE #2 — success path always refused ("PAYBOX : ID3D inconnu", CODEREPONSE 00041).** 3DS (RemoteMPI) = `preprod-tpeweb` merchant 222 → SITE **1999887**, but `E_TransactionPayment` DEBUG was sending the PPPS capture with **SITE=1999888** (a prior session "fixed a typo" the WRONG way: 1999887→1999888). ID3D minted under 1999887 is unknown when capture queries 1999888. Brute-forced all endpoint×site combos: **SITE=1999887 → CODEREPONSE=00000 on BOTH `recette-ppps` AND `preprod-ppps`; SITE=1999888 → ID3D inconnu on both.**
  - **FIX #2:** reverted deployed `www_2/src/payment/E_Transaction/E_TransactionPayment.php` DEBUG `PBX_SITE` → **`1999887`** (URL stays `recette-ppps.e-transactions.fr/PPPS.php`). Local repo copy already had 1999887.
  - ✅ **VERIFIED END-TO-END (clean build):** card `4000 0000 0000 1000` exp `12/30` cvc `123` → ID3D → twelvy_validate → **PPPS auth+capture CODEREPONSE=00000 "Demande traitée avec succès"** → redirect `status=ok` → **`/api/payment/status` returns `"paye"` (facture 274091)**. The card was NEVER the problem — the two server bugs were.
  - Cleaned up: all `twelvy_diag*.php` + instrumentation removed from live validate; 10 `diag-*@twelvy.net` test prospects soft-deleted.
  - **🔴 PROD CUTOVER (when `twelvy_env.php` DEBUG=false):** (a) to re-enable PSP emails, deploy real `mails_v3/*` + `modules/module.php` (Composer `vendor/` + `\Core\Log`) and verify SMTP from the prod host — OR keep them DEBUG-skipped and have Twelvy send its own; (b) PROD PPPS creds are the `else` branch (`0966892`/`02`/`651027368`/prod key, `ppps.paybox.com`) — verify with Verifone; (c) the SITE=1999887 fix is SANDBOX-only, does not touch prod.
