...
if (isset($_POST['action']) && !empty($_POST['action'])) {
    ...
    switch ($action) {
        ...
        case 'annulation_inscription':
            ...
            annulation_inscription($id_stagiaire, $motif);
...
function send_message_contact($id_stagiaire, $message)
{
    ...
    send_notification(1, $id_stagiaire, 0, "Message au service client : " . $message);
}
...
function enregistre_iban($id_stagiaire, $iban, $bic)
{
    ...
    $result = $profileRepo->updateOrCreate($id_stagiaire, [
...
function status_paiement($id_stagiaire)
{
    ...
    if ($ajout_paiement) {
        ...
        $stage = (new RetrieveFullOrderByStageStudent())->__invoke($id_stagiaire, $stageId, $mysqli);
        if ($stage) {
...
function demande_remboursement($id_stagiaire, $select_motif_annulation)
{
    ...
    $remb_prio = "";
    if ($row['provenance_suppression'] == 1) {
        // Annulation Centre, remb prioritaire
        $remb_prio = ", remboursement_prioritaire = 1 ";
    }
    ...
    require_once('../mails_v3/mail_demande_remboursement_stagiaire.php');
    mail_demande_remboursement($id_stagiaire);

    if ($supprime == 0) {
        require_once('../mails_v3/mail_annulation_centre2.php');
    ...
    if (strlen($select_motif_annulation) > 0 && $select_motif_annulation != "Veuillez saisir un motif d'annulation") {
        $message = "Demande de remboursement. Motif: " . $select_motif_annulation;
    } else {
        if ($row['provenance_suppression'] == 1) {
            $message = "Demande de remboursement. Motif: Annulation Centre";
        } else {
            if ($row['provenance_suppression'] == 1) {
                $message = "Demande de remboursement. Motif: Ancienne mise en attente";
            } else {
...
function transfert($reference, $old_stage, $new_stage, $prix, $stage_price = 0)
{
    ...
    $oldStageInfos = (new RetrieveStageTransfert())->__invoke($old_stage, $mysqli);
    ...
    if ($lastPaiement < $newStagePrice) {
        $ajout_paiement = $newStagePrice - $lastPaiement;
        $paiement       = $newStagePrice;
    }

    if ($lastPaiement > $newStagePrice) {
        $price_transfer     =   $lastPaiement - $newStagePrice;
    ...
    $messageStagiaire = "Transfert de stage";
    $message = "Transfert de stage";

    $log->info(
        'Transfert de stage',
        'transfer',
        LOGS . DS . 'transfer.log',
        false,
        [
    ...
    $message .= "Ancien: $oldStageStudent->membre_nom - $old_stage - $old_date_text $oldStageStudent->code_postal $oldStageStudent->ville - Prix: $oldStageStudent->paiement € <br> => Nouveau: $newStage->membre_nom - $new_stage - $new_date_text $newStage->code_postal $newStage->ville - Prix: $newStage->prix € => Nouvelle commission : $newStage->commission_ht | Ancienne commission : $oldStageInfos->commission_ht";
    ...
    send_notification($type_interlocuteur, $id_stagiaire, $notifie, $message, 0);
    send_notification($type_interlocuteur, $id_stagiaire, $notifie, $messageStagiaire);

    ob_start();
    if ($oldStageStudent->id_membre  == 1060) {
        if ($oldStageStudent->id_membre == $newStage->id_membre) {
            include("/home/prostage/www/ws/prod/fsp/to/inscription/edit.php");
            editInscription($oldStageStudent->id_membre, $id_stagiaire);
        } else {
            include("/home/prostage/www/ws/prod/fsp/to/inscription/cancel.php");
            cancelInscription($oldStageStudent->id_membre, $id_stagiaire);
        }
    } else {
        if ($newStage->id_membre  == 1060) {
            include("/home/prostage/www/ws/prod/fsp/to/inscription/add.php");
            addInscription($newStage->id_membre, $id_stagiaire, "TRANSFERT");
        }
    }


    //if ($oldStageStudent->id_membre  == 970) {
    $id_membre = $oldStageStudent->id_membre;
    if ($oldStageStudent->id_membre != $newStage->id_membre) {
        include("/home/prostage/www/ws/prod/psp/to/inscription/cancel.php");
        cancelInscription($id_membre, $id_stagiaire);
    } else {
        include("/home/prostage/www/ws/prod/psp/to/inscription/edit.php");
        editInscription($id_membre, $id_stagiaire);
    }
    //}
    $output = ob_get_clean();
}

function validateInscriptionFromProspect(
    $reference,
    $autorisation,
    $numappel,
    $numtrans,
    ...
    $transaction = mysqli_fetch_object($mysqli->query($sqlTransaction));

    if ($transaction) {
        $stageId = $new_stage;
        $id_stagiaire = $transaction->id_stagiaire;
        $sqlStage = "SELECT site.departement,stage.commission_ht,stage.partenariat, stage.id_membre FROM stage, site WHERE stage.id_site = site.id AND stage.id = '$stageId'";
        $stage = mysqli_fetch_object($mysqli->query($sqlStage));

        $dep = $stage->departement;
        $partenariat = $stage->partenariat;
        $commission_ht = $stage->commission_ht;
        $id_membre = $stage->id_membre;
        $provenance = 0;

        $sqlUpdateTransaction = "UPDATE transaction SET type_paiement='CB_OK', autorisation='$autorisation', id_stage = '$stageId', id_membre = '$id_membre', paiement_interne = 1 WHERE id=$reference";
        $mysqli->query($sqlUpdateTransaction);

        $sqlFacture = 'INSERT INTO facture_id(id_stagiaire) VALUES (' . $id_stagiaire . ')';
        $mysqli->query($sqlFacture);
        $I_idFacture = $mysqli->insert_id;

        // Modification de la requete en ajoutant la valeur facture_num
        $sqlUpdateStagiaire = "UPDATE stagiaire SET  
					supprime=0, 
					status='inscrit', 
					numero_cb = '$numero_cb',
					numappel = '$numappel', 
					...
					facture_num  = $I_idFacture
				WHERE id=$id_stagiaire";

        $mysqli->query($sqlUpdateStagiaire);

        (new UpdateStageAfterPayment())->__invoke($stageId, $mysqli);

        require_once("/home/prostage/www/mails_v3/mail_inscription.php");
        mail_inscription($id_stagiaire);

        require_once("/home/prostage/www/mails_v3/mail_inscription_centre.php");
        mail_inscription_centre($id_stagiaire);
    }
}
...
function send_notification($type_interlocuteur, $id_interlocuteur, $notifie, $message, $type_destinataire = 1)
{
...
function annulation_inscription($id_stagiaire, $motif)
{
    ...
    $sql = "UPDATE stagiaire SET supprime=1, attente=1, attente_remboursement=0, motif_annulation = '$motif' WHERE id = $id_stagiaire";
    mysql_query($sql, $stageconnect) or die(mysql_error());
    ...
    require_once('../mails_v3/mail_mise_en_attente_stagiaire.php');

    mail_mise_en_attente_stagiaire($id_stagiaire, $supprime);
...
function update_validations_stagiaire($id_stagiaire, $accordeon)
{
    ...
    if ($accordeon == 1) {
        ...
        if (intval($row_stagiaire['etat_permis']) == 1) { //valide
            if (
                !intval($row_stagiaire['type_permis']) ||
