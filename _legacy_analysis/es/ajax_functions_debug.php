...
if(isset($_POST['action']) && !empty($_POST['action'])) {
    ...
    switch($action) {
		...
		case 'send_notification':
			...
			send_notification($type_interlocuteur, $id_interlocuteur, $notifie, $message);
		break;
		...
		case 'rappel_identifiants':
			$id_stagiaire = $_POST['id_stagiaire'];
			$ret = rappel_identifiants($id_stagiaire);
			echo $ret;
		break;		
...
function status_stagiaire($id_stagiaire) {
	...
	else if ($supprime == 1 && $paiement > 0 && $remboursement == 0 && strlen($numtrans))
		$enabled = true;
	...
	echo json_encode(array('enabled' => $enabled, 'texte' => $texte));
...
function demande_remboursement($id_stagiaire, $select_motif_annulation) {
	...
	$sql = "UPDATE stagiaire SET attente_remboursement=1, motif_annulation=\"$select_motif_annulation\" WHERE id=$id_stagiaire";
...
function rappel_identifiants($id_stagiaire) {
	...
	$sql = "SELECT email FROM stagiaire WHERE id=$id_stagiaire";
	...
	if ($total) {
		phpmailer($email, $subject, utf8_decode($msg));
		return 1;
	}
	else
		return 0;
...
function transfert($reference, $old_stage, $new_stage, $prix) {
	...
	if ($old_membre != $new_membre) {
		include ("/home/prostage/www/mails_v3/mail_annulation_centre.php");
	...
	if ($old_membre != $new_membre) {
		...
		echo "1";
		include("/home/prostage/gae/functions.php");
		echo "2";
		...
		include ("/home/prostage/www/mails_v3/mail_transfert_stagiaire.php");
		...
		if ($new_membre == 793) {
			$ret = rppcTransfert($id_stagiaire_externe, $id_new_stage_externe);
			if (!$ret) {
	...
	$message .= "Ancien: $old_stage - $old_date $old_cp $old_ville - Prix:$old_paiement € <br> => Nouveau: $new_stage - $new_date $new_cp $new_ville - Prix:$new_prix €";
	send_notification($type_interlocuteur, $id_stagiaire, $notifie, $message);	

}

function rppcTransfert($id_stagiaire_externe, $id_stage_externe) {
...
function affiche_formulaire_paiement($id_stage, $id_stagiaire, $prix) {
	...
	if (($annule != 0) || ($nb_places_allouees <= 0) || (strtotime($date1) <= strtotime(date('Y-m-d')))) {
		...
		if ($reste_a_payer < 0) {
			...
			$bouton_paiement = "<button id_stagiaire='$id_stagiaire' old_stage='$old_stage' new_stage='$id_stage' numtrans='$numtrans' numappel='$numappel' reste_a_payer='$montant_remboursement' class='btn btn-primary remboursement_transfert_stage'>Transfert & Remboursement ".(-$reste_a_payer)." €</button>";
			...
			$credit_card_disabled = true;
			$bouton_paiement = "<button id_stagiaire='$id_stagiaire' prix='$prix' old_stage='$old_stage' new_stage='$id_stage' id_transaction='$id_transaction' class='btn btn-primary transfert_stage'>Transfert</button>";
			...
			$bouton_paiement = "<button id_stagiaire='$id_stagiaire' prix='$prix' old_stage='$old_stage' new_stage='$id_stage' class='btn btn-primary paiement_cb'>Payer $reste_a_payer €</button>";
				...
				$c .= "<div class='row'>";
					...
					$c .= "<div class='col-md-6'>";
						...
						$c .= "<input type='hidden' id='hidden_transaction' value='$id_transaction'>";
							...
							$c .= "<div class='row'>";
								$c .= "<div class='col-xs-12 col-md-5'>";
									$c .= $bouton_paiement;
								$c .= "</div>";
								
								$c .= "<div class='hidden-xs col-md-7' style='font-weight:bold;font-size:11px;font-style:italic'>";
...
function send_notification($type_interlocuteur, $id_interlocuteur, $notifie, $message) {
	...
	$type_destinataire = 1;
	notification($type_interlocuteur, $id_interlocuteur, $type_destinataire, $notifie, $message);
}
