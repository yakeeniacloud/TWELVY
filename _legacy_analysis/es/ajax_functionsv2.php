...
if (isset($_POST['action']) && !empty($_POST['action'])) {
    ...
    switch ($action) {
        ...
        case 'formulaire_paiement':
            ...
            affiche_formulaire_paiement($id_stage, $id_stagiaire, $prix);
            break;
        ...
        case 'paiement_cb':
            ...
            $new_stage = $_POST['new_stage'];
            $prix = $_POST['prix'];
            paiement_cb($reference, $montant, $cardNumber, $cardExpiry, $cardCVC, $k, $old_stage, $new_stage, $prix);
            break;
        ...
        case 'rappel_identifiants':
            $email = $_POST['email'];
            $login = $_POST['login'];
            $ret = rappel_identifiants($email, $login);
            echo $ret;
            break;
        ...
        case 'checkValidateDoc':
            $id_stagiaire = $_POST['id_stagiaire'];
            checkValidateDoc($id_stagiaire);
            break;
    }
}
...
function rappel_identifiants($email, $login)
{
    ...
    $sql = "SELECT id FROM stagiaire WHERE stagiaire.email = '$email' AND paiement > 0 AND id = '$login' ORDER BY id DESC LIMIT 1";
    $rs = mysql_query($sql, $stageconnect) or die(mysql_error());
    $total = mysql_num_rows($rs);
    $row = mysql_fetch_assoc($rs);
    $id_stagiaire = $row['id'];
...
function paiement_cb($reference, $montant, $cardNumber, $cardExpiry, $cardCVC, $k, $old_stage, $new_stage, $prix)
{
    ...
    if ($md5 != $k) {
        ...
        if (intval($ret[0]) != 0)
            transfert($reference, $old_stage, $new_stage, $prix);
...
function transfert($reference, $old_stage, $new_stage, $prix)
{
    ...
    $ajout_paiement = 0;
    if ($old_paiement < $new_prix) {
        $ajout_paiement = $new_prix - $old_paiement;
    }
    ...
    if ($old_membre != $new_membre) {
        require_once("/home/prostage/www/mails_v3/mail_annulation_centre2.php");
    ...
    $trainingApi->updateDataStageApi($old_stage);
    $trainingApi->updateDataStageApi($new_stage);
    ...
    if ($old_membre  == 1060) {
        if ($old_membre != $new_membre) {
            ...
            cancelInscription($old_membre ,$id_stagiaire);
        }else{
            include("/home/prostage/www/ws/prod/fsp/to/inscription/edit.php");
            editInscription($old_membre,$id_stagiaire);
        } 
    }
    //if ($old_membre  == 970) {
        if ($old_membre != $new_membre) {
            ...
            include("/home/prostage/www/ws/prod/psp/to/inscription/edit.php");
            editInscription($old_membre,$id_stagiaire);
        } 
    //}
}
...
function affiche_formulaire_paiement($id_stage, $id_stagiaire, $prix)
{
    ...
    if (empty($numappel) || !strlen($numappel) || is_null($numappel) || empty($numtrans) || !strlen($numtrans) || is_null($numtrans))
        $paiement = 0;
    else
    ...
    if (($annule != 0) || ($nb_places_allouees <= 0) || (strtotime($date1) <= strtotime(date('Y-m-d')))) {
        ...
        $bouton_paiement = "<button disabled class='btn btn-primary' style='width:240px;'>Payer</button>";
    } else {
        ...
        if ($reste_a_payer < 0) {
            $credit_card_disabled = true;
            $montant_remboursement = -$reste_a_payer;
            $bouton_paiement = "<button id_stagiaire='$id_stagiaire' old_stage='$old_stage' new_stage='$id_stage' numtrans='$numtrans' numappel='$numappel' reste_a_payer='$montant_remboursement' class='btn btn-primary remboursement_transfert_stage' style='width:240px;'>Transfert & Remboursement " . (-$reste_a_payer) . " €</button>";

            $texte1 = "Vous avez un avoir de " . $avoir . "€. Vous pouvez être transféré sur ce nouveau stage et être remboursé de la différence soit " . (-$reste_a_payer) . "€.";

            $texte2 = "Cliquez sur le bouton “Remboursement” pour valider votre transfert et demander le remboursement de la différence.";
        } else if ($reste_a_payer == 0) {
            $credit_card_disabled = true;
            $bouton_paiement = "<button id_stagiaire='$id_stagiaire' prix='$prix' old_stage='$old_stage' new_stage='$id_stage' id_transaction='$id_transaction' class='btn btn-primary transfert_stage' style='width:240px;'>Transfert</button>";
            ...
            $bouton_paiement = "<button id_stagiaire='$id_stagiaire' prix='$prix' old_stage='$old_stage' new_stage='$id_stage' class='btn btn-primary paiement_cb' style='width:240px;'>Payer $reste_a_payer €</button>";

            $texte1 = "Ce stage est plus cher que celui que vous avez réservé car, les prix peuvent varier en fonction des dates et des lieux. Pour finaliser votre réservation, vous devez régler un complément de " . $reste_a_payer . " €.";
    ...
    $c .= "<div class='row'>";
    $c .= "<div class='col-xl-6 col-lg-6 col-md-6 col-sm-6 col-xs-12'>";
    ...
    $c .= "<p class='m-t' style='text-align:justify'>";
    $c .= $texte1;
    ...
    $c .= "<div class='col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12'>";
    ...
    $c .= "<div class='row'>";
    $c .= "<div class='col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12' style='text-align:center;'>";
    $c .= $bouton_paiement;
    $c .= "</div>";

    $c .= "<div class='col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12' style='font-weight:bold;font-size:11px;font-style:italic;text-align:center;'>";
    $c .= "Transaction bancaire totalement sécurisée par système de cryptage SSL 128 bits";
    $c .= "</div>";
    $c .= "</div>";
    $c .= "</div>";
...
function update_validations_stagiaire($id_stagiaire, $accordeon)
{
    ...
    if ($accordeon == 1) {
        ...
        if (
            intval($row_stagiaire['etat_permis']) == 1 ||
            intval($row_stagiaire['etat_permis']) == 2 ||
            intval($row_stagiaire['etat_permis']) == 3 ||
            intval($row_stagiaire['etat_permis']) == 6 ||
            intval($row_stagiaire['etat_permis']) == 7
        ) { //valide
...
function update_sous_cat_etat_permis($new_etat_permis, $id_stagiaire, $echo)
{
    ...
    $date_permis = trim($row['date_permis']);
    $tmp = explode('-', $date_permis);
    if (count($tmp) == 3 && checkdate($tmp[1], $tmp[2], $tmp[0])) {
        ...
        $date_permis_format = '<span style="color: red; font-style: italic">À renseigner</span>';
    }
    ...
    $selectPermisValue = '<span style="color: #585858; font-style: italic">Non renseigné</span>';
    switch ($etat_permis) {
        case 0 :
            $selectPermisValue = '<span style="color: #585858; font-style: italic">Non renseigné</span>';
            break;
        case 1 :
            $selectPermisValue = 'VALIDE';
            break;
        case 2 :
            $selectPermisValue = 'RETENTION DE 72H';
            break;
        case 3 :
            $selectPermisValue = 'SUSPENSION';
            break;
        case 4 :
            $selectPermisValue = 'ANNULE OU INVALIDE';
            break;
        case 5 :
            $selectPermisValue = 'JAMAIS OBTENU';
            break;
        case 6 :
            $selectPermisValue = 'PERDU';
            break;
        case 7 :
            $selectPermisValue = 'VOL';
            break;
    }
    ...
    if ($date_permis) {
        $arrDate = explode('-', $date_permis);
        $permis_day = $arrDate[2];
        $permis_month = $arrDate[1];
        $permis_year = $arrDate[0];
    }

    $optionsJourPermis = '<option value="">Jour</option>';
    ...
    for ($i = 1; $i <= 31; $i++) {
        ...
        $optionsJourPermis .= '<option value="' . $i . '" ' . $selectedDay . '>' . $i . '</option>';
    }

    $outputDatePermis = "<select  class='form-control select_date_permis' name='permis_day' id='permis_day'>" . $optionsJourPermis . "</select><select class='form-control select_date_permis' name='permis_month' id='permis_month'>" . buildMonthSelect($permis_month, true) . "</select><select class='form-control select_date_permis' name='permis_year' id='permis_year'>" . buildYearSelect($permis_year) . "</select>";

    $c = '
        <div class="row rv1">
            <div class="col-md-4 col-sm-4 col-xs-4 nolrp"><span class="label_text">Votre permis est</span></div>
            <div class="col-md-5 col-sm-5 col-xs-5  nolrp">
            	<span id="etat_permis">' . $selectPermisValue . '</span>
            	<div class="div_input"></div>
            </div>
            <div class="col-md-3 col-sm-3 col-xs-3 nolrp">
            	<span class="update_field_btn update_etat_permis" field="etat_permis"  id="' . $id_stagiaire . '" oldValue="' . $etat_permis . '">Modifier</span>
                
                 <div class="div_input_btn"></div>
			</div>
        </div>      
    ';

    $selectTypePermisProbatoire = '<span style="color: #585858; font-style: italic">Non renseigné</span>';
    switch ($permis_probatoire) {
        case '' :
            $selectTypePermisProbatoire = '<span style="color: #585858; font-style: italic">Non renseigné</span>';
            break;
        case 1 :
            $selectTypePermisProbatoire = 'Oui';
            break;
        case 2 :
            $selectTypePermisProbatoire = 'Non';
            break;
    }

    $selectPointPermis = '<span style="color: #585858; font-style: italic">Non renseigné</span>';
    switch ($points_restant) {
        case '' :
            $selectPointPermis = '<span style="color: #585858; font-style: italic">Non renseigné</span>';
            break;
        case 1 :
            $selectPointPermis = '0 point';
            break;
        case 2 :
            $selectPointPermis = '1 point';
            break;
        case 3 :
            $selectPointPermis = '2 points';
            break;
        case 4 :
            $selectPointPermis = '3 points';
            break;
        case 5 :
            $selectPointPermis = '4 points';
            break;
        case 6 :
            $selectPointPermis = '5 points';
            break;
        case 7 :
            $selectPointPermis = '6 points';
            break;
        case 8 :
            $selectPointPermis = '7 points';
            break;
        case 9 :
            $selectPointPermis = '8 points';
            break;
        case 10 :
            $selectPointPermis = '9 points';
            break;
        case 11 :
            $selectPointPermis = '10 points';
            break;
        case 12 :
            $selectPointPermis = '11 points';
            break;
        case 13 :
            $selectPointPermis = '12 points';
            break;
        case 14 :
            $selectPointPermis = 'je ne sais pas';
            break;
    }
    ...
    switch ($new_etat_permis) {
        ...
        case 6:
        case 7:
            $selectTypePermisValue = '<span style="color: #585858; font-style: italic">Non renseigné</span>';
            switch ($type_permis) {
                case 0 :
                    $selectTypePermisValue = '<span style="color: #585858; font-style: italic">Non renseigné</span>';
                    break;
                case 1 :
                    $selectTypePermisValue = 'Permis rose';
                    break;
                case 2 :
                    $selectTypePermisValue = 'Carte à puce';
                    break;
            }

            $c .= '
              <div class="row rv1">
                  <div class="col-md-4 col-sm-4 col-xs-4 nolrp"><span class="label_text">Type de permis</span></div>
              		<div class="col-md-5 col-sm-5 col-xs-5  nolrp">
						<span id="type_permis" data-value="' . $type_permis . '">' . $selectTypePermisValue . '</span>
						<div class="div_input"></div>
					</div>
					<div class="col-md-3 col-sm-3 col-xs-3 nolrp">
						<span class="update_field_btn update_type_permis" field="type_permis"  id="' . $id_stagiaire . '" oldValue="' . $type_permis . '">Modifier</span>
						<div class="div_input_btn"></div>
				  </div>
              </div>  

              <div class="row margin-tb row_date_permis">
                  <div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-xs-4 nolrp">
                        <span class="label_text">Date délivrance <br>permis <span style="color: red">*</span></span>
                  </div>
                  <div class="col-sm-5 col-xs-5 col-md-5 nolrp">
                       <span id="date_permis" style="margin-right: 10px">' . $date_permis_format . '</span>
                       ' . $outputDatePermis . '
                  </div>
                  <div class="col-md-3 col-md-xs-3 col-sm-3 col_btn_date_permis" style="padding: 0">
                        <span class="update_field_btn" id="selected_date_permis" field="date_permis"  id="' . $id_stagiaire . '" oldValue="' . $date_permis . '">Modifier</span>
                       <button class="btn btn-validate btn_update_date_permis" id="update_date_permis" idStudent="' . $id_stagiaire . '">
                        Valider
                       </button>
                   </div>
             </div>
             ...
             <div class="row rv1">
                 <div class="col-md-4 col-sm-4 col-xs-4 nolrp">
                     <div><span class="label_text">Numéro permis</span><span style="color: red">*</span></div>
                 </div>
                 <div class="col-md-5 col-sm-5 col-xs-5 nolrp">
                      <span id="num_permis">' . ($num_permis ? $num_permis : '<span style="color: red; font-style: italic">À renseigner</span>') . '</span>
                      <div class="div_input"></div>
                 </div>
                 <div class="col-md-3 col-sm-3 col-xs-3 nolrp">
                     <span class="update_field_btn update_text" field="num_permis" id="' . $id_stagiaire . '" oldValue="' . $num_permis . '">Modifier</span>
                    <div class="div_input_btn"></div>
                </div>
             </div>
             ...
             <div class="row margin-tb">
                 <div class="col-md-4 col-sm-4 col-xs-4 nolrp">
                     <div><span class="label_text">Département <br>délivrance permis</span> <span style="color: red">*</span></div>
                 </div>
                 <div class="col-md-5 col-sm-5 col-xs-5 nolrp">
                      <span id="lieu_permis" data-value="' . $lieu_permis . '">' . ($lieu_permis ? $lieu_permis : '<span style="color: red">À renseigner</span>') . '</span>
                      <div class="div_input"></div>                
                 </div>
                 <div class="col-md-3 col-sm-3 col-xs-3 nolrp">
                     <span class="update_field_btn update_lieu_permis" field="lieu_permis" id="' . $id_stagiaire . '" oldValue="' . $lieu_permis . '">Modifier</span>
                    <div class="div_input_btn"></div>
                </div>
             </div>
             
             <div class="row margin-tb">
                  <div class="col-md-4 col-sm-4 col-xs-4 nolrp"><span class="label_text">Etes vous conducteur en permis probatoire ?</span></div>
              		<div class="col-md-5 col-sm-5 col-xs-5  nolrp">
						<span id="permis_probatoire" data-value="' . $permis_probatoire . '">' . $selectTypePermisProbatoire . '</span>
						<div class="div_input"></div>
					</div>
					<div class="col-md-3 col-sm-3 col-xs-3 nolrp">
						<span class="update_field_btn update_permis_probatoire" field="permis_probatoire"  id="' . $id_stagiaire . '" oldValue="' . $permis_probatoire . '">Modifier</span>
						<div class="div_input_btn"></div>
				  </div>
              </div>  
                
              <div class="row margin-tb">
                  <div class="col-md-4 col-sm-4 col-xs-4 nolrp"><span class="label_text">Nombre de points restant</span></div>
              		<div class="col-md-5 col-sm-5 col-xs-5  nolrp">
						<span id="points_restant" data-value="' . $points_restant . '">' . $selectPointPermis . '</span>
						<div class="div_input"></div>
					</div>
					<div class="col-md-3 col-sm-3 col-xs-3 nolrp">
						<span class="update_field_btn update_point_restant" field="points_restant"  id="' . $id_stagiaire . '" oldValue="' . $points_restant . '">Modifier</span>
						<div class="div_input_btn"></div>
				  </div>
              </div>    
             
             <div class="row margin-tb">
              <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 nolrp">
                 <span style="color: red; font-style: italic"> * Champ obligatoire</span>
              </div>
            </div>
        ...
        case 4:
            ...
            $selectReception48SI = '<span style="color: #585858; font-style: italic">Non renseigné</span>';
            switch ($reception_48SI) {
                case 0 :
                    $selectReception48SI = '<span style="color: #585858; font-style: italic">Non renseigné</span>';
                    break;
                case 1 :
                    $selectReception48SI = 'Oui';
                    break;
                case 2 :
                    $selectReception48SI = 'Non';
                    break;
            }
            ...
            $c .= '
              ...
              <div class="row rv1">
                  <div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-xs-4 nolrp"><span class="label_text">Avez-vous receptionne la lettre recommandee reference 48SI</span><br> <span style="font-style: italic; font-size: 11px">(permis invalide pour solde nul) ?</span></div>
              		<div class="col-md-5 col-sm-5 col-xs-5 nolrp">
						<span id="reception_48SI" data-value="' . $reception_48SI . '">' . $selectReception48SI . '</span>
						<div class="div_input"></div>
					</div>
					<div class="col-md-3 col-sm-3 col-xs-3 nolrp">
						<span class="update_field_btn update_reception_48SI" field="reception_48SI"  id="' . $id_stagiaire . '" oldValue="' . $reception_48SI . '">Modifier</span>
						<div class="div_input_btn"></div>
				  </div>
              </div>  
...
function editable($id, $name, $value)
{
    ...
    echo json_encode(updateValidateStagiaireData($id));
}
...
function update_cas($id, $name, $value, $selects)
{
    ...
    updateValidateStagiaireData($id);
...
function espace_stagiaire_telechargement_documents($id_stagiaire, $reload = false)
{
    ...
    $md5 = substr($md5, 0, 10);
    $src = HOST . "/stages/telechargement_documentsv2.php?id=" . $id_stagiaire . "&k=" . $md5 . "&preventcache=" . $time;

    $c = "";

    $c .= "<div style='overflow: hidden;'>
		<iframe id='iframe_telechargement_documents' scrolling='no' src='$src'>
		</iframe>
		</div>";
...
function get_text_cas($cas, $selects)
{
    ...
    switch ($cas) {
        ...
        default:
            $text = "Sélectionnez votre cas de stage pour voir apparaitre les informations importantes concernant votre situation";
            break;
    }
...
function buildMonthSelect($month, $isPermis = null)
{
    if ($isPermis) {
        $optionsMois = "<option value=''>Mois</option>";
        $optionsMois .= "<option value='1'  " . ($month == 1 ? "selected" : "") . ">01</option>";
        $optionsMois .= "<option value='2' " . ($month == 2 ? "selected" : "") . ">02</option>";
        $optionsMois .= "<option value='3' " . ($month == 3 ? "selected" : "") . ">03</option>";
        $optionsMois .= "<option value='4' " . ($month == 4 ? "selected" : "") . ">04</option>";
        $optionsMois .= "<option value='5' " . ($month == 5 ? "selected" : "") . ">05</option>";
        $optionsMois .= "<option value='6' " . ($month == 6 ? "selected" : "") . ">06</option>";
        $optionsMois .= "<option value='7' " . ($month == 7 ? "selected" : "") . ">07</option>";
        $optionsMois .= "<option value='8' " . ($month == 8 ? "selected" : "") . ">08</option>";
        $optionsMois .= "<option value='9' " . ($month == 9 ? "selected" : "") . ">09</option>";
        $optionsMois .= "<option value='10' " . ($month == 10 ? "selected" : "") . ">10</option>";
        $optionsMois .= "<option value='11' " . ($month == 11 ? "selected" : "") . ">11</option>";
        $optionsMois .= "<option value='12' " . ($month == 12 ? "selected" : "") . ">12</option>";
        return $optionsMois;
    }
    $optionsMois = "<option value=''>Mois</option>";
    $optionsMois .= "<option value='1'  " . ($month == 1 ? "selected" : "") . ">Janvier</option>";
    $optionsMois .= "<option value='2' " . ($month == 2 ? "selected" : "") . ">Février</option>";
    $optionsMois .= "<option value='3' " . ($month == 3 ? "selected" : "") . ">Mars</option>";
    $optionsMois .= "<option value='4' " . ($month == 4 ? "selected" : "") . ">Avril</option>";
    $optionsMois .= "<option value='5' " . ($month == 5 ? "selected" : "") . ">Mai</option>";
    $optionsMois .= "<option value='6' " . ($month == 6 ? "selected" : "") . ">Juin</option>";
    $optionsMois .= "<option value='7' " . ($month == 7 ? "selected" : "") . ">Juillet</option>";
    $optionsMois .= "<option value='8' " . ($month == 8 ? "selected" : "") . ">Août</option>";
    $optionsMois .= "<option value='9' " . ($month == 9 ? "selected" : "") . ">Septembre</option>";
    $optionsMois .= "<option value='10' " . ($month == 10 ? "selected" : "") . ">Octobre</option>";
    $optionsMois .= "<option value='11' " . ($month == 11 ? "selected" : "") . ">Novembre</option>";
    $optionsMois .= "<option value='12' " . ($month == 12 ? "selected" : "") . ">Décembre</option>";
    return $optionsMois;
}
...
function buildYearSelect($year)
{
    ...
    while ($iCurrentYear > $iStartYear) {
        ...
        $optionsYear .= "<option value='" . $Y . "' " . $selectedYear . ">" . $Y . "</option>";
        $iCurrentYear = strtotime("-1 year", strtotime($currentDate));
    }
    return $optionsYear;
}

function updateValidateStagiaireData($id_stagiaire)
{
    require_once("/home/prostage/connections/config.php");
    require_once "/home/prostage/www/params.php";
    require_once "/home/prostage/www/debug.php";
    require_once APP . 'student/services/UpdateValidationStudentDataAfterUpdate.php';

    return (new UpdateValidationStudentDataAfterUpdate())->execute($id_stagiaire, $mysqli);
}

function checkValidateDoc($id_stagiaire)
{
    require_once("/home/prostage/www/stages/functions.php");
    include("/home/prostage/connections/stageconnect.php");
    mysql_select_db($database_stageconnect, $stageconnect);
    ...
    $missings = documentsMissing($id_stage, $date_stage, $id_stagiaire, $row_stagiaire['cas'], $row_stagiaire['etat_permis'], $row_stagiaire['solde_nul'], $row_stagiaire['date_48n']);
    if (count($missings)) $documentInformation = false;
    echo $documentInformation == true ? 1 : 0;
}
