...
function inscription($id_stage, $id_membre)
{
	...
	$query_stage =
		...
		SELECT
			...
			site_dyn.ville,
			site_dyn.adresse,
			site_dyn.code_postal,
			site_dyn.id_membre,
			membre.id AS membreID,
			membre.mode

		FROM
			stage_dyn, site_dyn, membre

		WHERE
			stage_dyn.id = $id_stage AND
			stage_dyn.id_site = site_dyn.id_externe AND
			stage_dyn.id_membre = site_dyn.id_membre AND
			stage_dyn.id_membre = $id_membre";
	...
	echo "<td><em>".$row_stage['nom']."<br>".$row_stage['adresse']."<br>".sprintf('%05d',$row_stage['code_postal'])." ".$row_stage['ville']."</em></td>";
	echo "</tr>";
...
n'est pas le cas, appelez nous pour réserver votre place par <strong><u>téléphone</u></strong>.</em></td></tr>";
	...
	if ($id_membre == 44 || $id_membre == 64 || $id_membre == 65) //securoute + actiroute + allopermis
	{
		echo "<tr>";
