...
function verif_boost($id_stage, $row_stage = NULL, $update = false) {
    ...
    if (! $row_stage) {
        $query_boost = "SELECT * FROM stage WHERE stage.id = '$id_stage'";
    ...
    if (empty($row_stage['option_reversement']))
    ...
    if (    !empty($row_stage['nb_places_allouees'])
        &&  $nb_boost_possible > 0
        &&  (empty($row_stage['annule']) || $row_stage['id_membre'] == 188)
    ...
    if ($row_stage['option_reversement'] == 2)
    {
        if (!$boost_possible) {
            ...
            if ($update) {
//                echo "update stage set nb_boost_allouees = $nb_boost_possible where id = '$id_stage'";
    ...
    return array('boost_possible' =>$boost_possible, 'is_boost'=> $row_stage['option_reversement'] == 2, 'option_reversement' =>$row_stage['option_reversement'], 'nb_jours_restants'=>$nb_jours_restants, 'nb_boost_possible'=>$nb_boost_possible, 'nb_boost'=>$row_stage['nb_boost'], 'nb_boost_allouees'=>$row_stage['nb_boost_allouees']);
}
...
function tableau_premium_departements($site, $departement)
{
	require_once ("../common/functions.php");
...
//        }
        ...
        $query_stage = "SELECT
                                ...
                                WHERE
                                        ...
                                        stage.visible = 1 AND
                                        nb_places_allouees > 0 AND
                                        (annule = 0 OR stage.id_membre = 188) AND
                                        stage.id_membre = membre.id AND
...
//        }
        ...
        do
        {
            ...
            switch($row_rsStage['option_visibilite']) {
                case '1' : $is_premium = 1; break;
                case '2' : $is_flash = 1; break;
                case '3' : $is_boost = 1; break;
            }
...
//                          if (isset($_GET['afficher_boost']) && $is_boost)
            ...
            else
            {
                ...
                echo '<span class="prix" itemprop="lowPrice">'.$prix.' &#128;</span>
                        <br /><span class="old_prix" itemprop="highPrice">'.$ancien_prix.(!empty($ancien_prix)? ' &#128;' : '').'</span>
                        <span style="display:none">
                            <span itemprop="offerCount">
                                '.$row_rsStage['nb_places_allouees'].'
                            </span> places
...
function affiche_stages_nouveau($site, $departement, $ville, $code_postal, $region, $chemin, $query_site, $query_stage, $iframe = false)
{
	//random du département si pas de stages dans une ville
	//-----------------------------------------------------
	$random = false;
...
// TODO : voir si MM_jumpMenu ici est bien utile !
